 ...
 *********************************************************************/
    ...
    public void visitMethodDef(JCMethodDecl tree) {
        ...
        annotate.annotateTypeLater(tree, localEnv, tree.sym, tree.pos());

        if (tree.defaultValue != null)
            annotateDefaultValueLater(tree.defaultValue, localEnv, tree.sym, tree.pos());
    }
    ...
    public void visitVarDef(JCVariableDecl tree) {
        ...
        if ((enclScope.owner.flags_field & FROMCLASS) != 0) {
            for (Symbol sym : enclScope.getSymbolsByName(tree.name)) {
                boolean sameType = (enclScope.owner.flags_field & APT_CLEANED) != 0 ? isSameAPType(tree.vartype.type, sym.type) : types.isSameType(tree.vartype.type, sym.type);
